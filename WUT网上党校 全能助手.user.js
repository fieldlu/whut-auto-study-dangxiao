...
// @version      1.6.0
// @description  全自动刷课：视频断点续播/智能跳课/进度看门狗/真人模拟，打开培训班页面后自动运行
// @author       FieldLu
// @license      MIT
...
'use strict';
  ...
  function getCourseProgress(course) {
    if (course.progress != null) return parseFloat(course.progress) || 0;
    const duration = parseFloat(course.duration) || 0;
    const finishedDuration = parseFloat(course.finishedDuration) || 0;
    return duration > 0 && finishedDuration >= duration ? 1 : 0;
  }

  function renderCourseListItems(courses) {
    if (!listEl || !Array.isArray(courses)) return;
    listEl.replaceChildren();
    courses.forEach(course => {
      const mode = course.studyMode || '';
      const progress = Math.max(0, Math.min(1, getCourseProgress(course)));
      const cls = progress >= 1 ? 'done'
        : (progress > 0 ? 'half' : (mode !== 'WLXX' && mode !== '' ? 'skip' : 'todo'));
      const icon = progress >= 1 ? '✔' : (progress > 0 ? '◐' : '○');
      const item = document.createElement('span');
      item.className = cls;
      item.textContent = icon + ' ' + String(course.courseName || course.name || '未命名课程')
  ...
  function renderCourseList() {
    ...
    if (courses.length === 0 && courseList.length > 0) {
      courses = courseList;
    }
  ...
  const API = {
    getXm: () => fetch('/api/student/xm/getById', {
      ...
      body: 'id=' + encodeURIComponent(xmId),
    }).then(r => r.json()),
    getCourse: (rid) =>
      fetch('/api/student/recommendCourse/getById?id=' + encodeURIComponent(rid)
        + '&xmId=' + encodeURIComponent(xmId)).then(r => r.json()),
  };
  ...
  function init() {
    ...
    if (gotoDetail) {
      ...
      startHumanSimulator();
      location.href = '#/myTrain/detail?id=' + encodeURIComponent(xmId);
      return;
    }
    ...
    if (gotoCourse) {
      ...
      try {
        ...
        startHumanSimulator();
        location.href = '#/myTrain/course?id=' + encodeURIComponent(c.resourceId)
          + '&xmId=' + encodeURIComponent(c.xmId);
      } catch(e) {}
      return;
    }
